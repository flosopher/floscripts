...
for(my $ii = 0; $ii < $NumRows; $ii++){
	...
	my @CurLineArray = split(' ',$DataStrings[$ii]);
	 ...
	 #first data line, check how many columns there are
...
for(my $rr = 0; $rr < $NumReqs; $rr++) {
	...
	if($CurSubmode eq '>'){
		...
		if($DataArray[$ii][$CurCol ] <= $CurValue){$DataArray[$ii][$NumColumns+1] = 0;}
	    }
	}
	elsif($CurSubmode eq '<'){
	    for(my $ii = 0; $ii < $NumEntries; $ii++){
...
#first StdDevs, 
printf STDERR "The following averages and Standard Deviations were obtained: \n Field                      Av     StdDev \n";
for(my $kk = 1; $kk < $NumColumns; $kk++){
    #if($req_columns[$kk] == 1) {
	...
	printf STDERR "%s    %.2f      %.2f\n", &Padsp($title_array[$kk], 20),$ColumnArray[0][$kk + 1], $CurStdDev;
    #}
}
printf STDERR "\n";
...
if( ($OutOption != 1) && ($ReducedOutput != 1) && ($NumCommentLines > 0)) {printf STDOUT "%s",$DataStrings[0];} #assuming the first line is the title line
  ...
  my $outstring = $title_array[0]." ";
  for(my $kk = 1; $kk < $NumColumns; $kk++){
    if( $req_columns[$kk] == 1) { $outstring = $outstring.$title_array[$kk]." "; }
  }
  printf STDOUT "%s\n",$outstring; 
}
...
for(my $ii = 0; $ii < $NumEntries; $ii++){  
	...
	if($OutOption == 1) { printf STDOUT "%s\n",$SortDataArray[$ii][0];} #only print out filenames
	  ...
	  my $outstring = $SortDataArray[$ii][0]." ";
	  for(my $kk = 1; $kk < $NumColumns; $kk++){
	    if( $req_columns[$kk] == 1) { $outstring = $outstring.$SortDataArray[$ii][$kk]." "; }
	  }
	  printf STDOUT "%s\n",$outstring;
	}
