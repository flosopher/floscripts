...
def DNA_selections(display=True):
	...
	cmd.select('DNA', 'r. g+a+c+t+gua+ade+cyt+thy+da+dc+dg+dt')
	cmd.select('notDNA','not DNA')
	cmd.select('DNAbases','DNA and not %s' % bbatoms )
	cmd.select('DNAbb','DNA and %s' % bbatoms )
	cmd.select('sc_base','byres notDNA w. 6 of DNAbases')
	cmd.select('sc_base','sc_base and not n. c+n+o')
	cmd.select('dna_h2o','%s w. 3.6 of DNAbases' %waters )
...
cmd.extend('DNAselections', lambda: DNA_selections(True) )
cmd.extend('DNAselections_nodisplay', lambda: DNA_selections(False) )
