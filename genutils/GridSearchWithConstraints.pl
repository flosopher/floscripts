...
for(my $xx = $GridXLowerBound;$xx <= $GridXUpperBound; $xx += $GridSpacing) {
	...
	for(my $zz = $GridZLowerBound; $zz <= $GridZUpperBound; $zz += $GridSpacing) {
		...
		if($CurOlgDist <= 1.7 && $CurOlgDist >= 1.4 ) {
	        #if($CurOlgDist => 0.1 ) { #line to exclude distance

		    my $CurOwatAng = &angle_3points($GridCenter[0], $GridCenter[1], $GridCenter[2], $OwatCoords[0], $OwatCoords[1], $OwatCoords[2], $xx, $yy, $zz);
		    if($CurOwatAng <= 120 && $CurOwatAng >= 100 ) {

			my $CurOlgAng = &angle_3points($GridCenter[0], $GridCenter[1], $GridCenter[2], $OlgCoords[0], $OlgCoords[1], $OlgCoords[2], $xx, $yy, $zz);
			if($CurOlgAng <= 120 && $CurOlgAng >= 100 ) {
...
foreach my $point (@AllowedPoints ) {
    ...
    printf STDOUT "HETATM%s  gp  GRD X   1     ",&spPad($pointnum,5); 
