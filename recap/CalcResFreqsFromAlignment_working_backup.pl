...
while($SeqFileLine=<SAFILE>) {
    ...
    else {substr($SeqArray[$NumAlSeqs-1],-1) = $SeqFileLine;}

}

close SAFILE;

printf STDOUT "${enzyme}: Alignment file with %s sequences read. %s\n",$NumAlSeqs, scalar @SeqArray;
...
for( my $dcount=0; $dcount<$NumDesPos; $dcount++) {
	...
	my $currAA = substr($SeqArray[$scount],$DesPosArray[$dcount]-1,1);
