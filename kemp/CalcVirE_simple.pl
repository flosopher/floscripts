...
while(<CATGEOFILE>){
    ...
    $f->{decoy} = $linearray[0];
    $f->{Elig} = $linearray[1];
    $f->{COdist} = $linearray[2];
    $f->{CHO_ang} = $linearray[3];
    $f->{HOB_ang} = $linearray[4];
    $f->{Dih} = $linearray[5];
    #$f->{alt_COdist} = $linearray[6];
    #$f->{alt_CHO_ang} = $linearray[7];
    #$f->{alt_HOB_ang} = $linearray[8];
    #$f->{alt_Dih} = $linearray[9];
    $f->{TotE} = $linearray[6];
    #$f->{comment} = "n/a";
...
for( my $i = 0; $i < $lowdecs; $i++ ) {
    ...
    if($decoyElist[$i]->{COdist} < 3.9) {
	if($decoyElist[$i]->{CHO_ang} > 130) {
	    if(abs($decoyElist[$i]->{HOB_ang} - 114) < 30) {
		if($decoyElist[$i]->{Dih} < 45 || $decoyElist[$i]->{Dih} > 135) {
   ...
   if($TotE_sortlist[$i]->{COdist} < 3.9) {
	if($TotE_sortlist[$i]->{CHO_ang} > 130) {
	    if(abs($TotE_sortlist[$i]->{HOB_ang} - 114) < 30) {
		if($TotE_sortlist[$i]->{Dih} < 45 || $TotE_sortlist[$i]->{Dih} > 135) {
...
printf SatDecFILE "%s decoys qualify\n",$DecLigESat;
foreach my $f (@Satdecs) {
    printf SatDecFILE "%s %.2f %.2f %.2f %.2f %.2f\n",$f->{decoy},$f->{Elig},$f->{COdist},$f->{CHO_ang},$f->{HOB_ang},$f->{Dih};
}
close(SatDecFILE);
}
...
printf BolOUTf "${design}  %.2f  %.2f\n", $LigEDecFrac, $TotEDecFrac;
