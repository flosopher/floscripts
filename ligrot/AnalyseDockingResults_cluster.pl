...
sub ReadDockOutputStructPDB {
    ...
    $ligrmsdline= readpipe "/users/flo/scripts/rms_cur.py -t $InputFile -p $CurFile  -s \"HET;element!=H\"";  
    $scrmsdline = readpipe "/users/flo/scripts/rms_cur.py -t $InputFile -p $CurFile  -s \"resi=${DesignPosRmsLine};element!=H\"";
...
$InputFile = "${RosettaCode}_${enzyme}_input.pdb";
$RepackFile = "${RosettaCode}_${enzyme}_input_repacked.pdb";
$MinFile = "${RosettaCode}_${enzyme}_input_min.pdb";
...
for(my $ii=1; $ii<=$numstruct; $ii++) {
    ...
    my $curfile = "${RosCode1}${enzyme}_${convert_ii}.pdb";
