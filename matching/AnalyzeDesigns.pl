...
for(my $ii = 0; $ii < $NumDesigns; $ii++){
	...
	if( ($NumRows - 2) > $MaxCat) {$MaxCat = $NumRows - 2;}  
...
#build header line
my @HeaderLine = ("# Design","bk_tot","Ecst","Erep_tot","lig_sum","lig_rep","lig_hbond");
for(my $ii=1; $ii <= $MaxCat; $ii++){
    $HeaderLine[6+$ii] = "CatRes".$ii." rep cst";
}

my $NumColumns = scalar @HeaderLine;

#header line built, now output
printf STDOUT "\n";
printf STDOUT "%s",&Padsp($HeaderLine[0],62);
for(my $ii = 1; $ii < 7; $ii++){ printf STDOUT "%s",&Padsp(&spPad($HeaderLine[$ii],9),10);}
for(my $ii = 7; $ii < $NumColumns; $ii++){ printf STDOUT "%s",&Padsp(&spPad($HeaderLine[$ii],16),17);}
printf STDOUT "\n";

#header line was spit out, now output designs
for(my $ii = 0; $ii < $NumDesigns; $ii++){
    printf STDOUT "%s",&Padsp($DesignDataArray[$ii][0],62);
    for(my $jj = 1; $jj < 7; $jj++){ printf STDOUT "%s",&Padsp(&spPad($DesignDataArray[$ii][$jj],9),10);}
    for(my $jj = 7; $jj < $NumColumns; $jj++){ printf STDOUT "%s",&Padsp(&spPad($DesignDataArray[$ii][$jj],16),17);}
    printf STDOUT "\n";
}
